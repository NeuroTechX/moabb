...
Enhancements
~~~~~~~~~~~~
- Add :class:`moabb.datasets.SensoryGuidedMI2026`, a 39-participant, four-class (left hand, right hand, up, down) longitudinal motor-imagery cursor-control dataset with 62-channel EEG at 1000 Hz across sensory-guided joint-learning, tactile-control, BCI2000-control, and independent EEGNet-control groups (by `Bruno Aristimunha`_)
- Add 9 new motor-imagery dataset adapters: :class:`moabb.datasets.Ma2022` (25 subjects, binary cross-session left/right-hand motor imagery across 5 sessions, 32-channel at 250 Hz), :class:`moabb.datasets.Ortiz2023` (10 subjects, 3-class cognitive assessment of motor imagery during lower-limb-exoskeleton walking — relax vs motor imagery vs mental counting, 27-channel at 200 Hz), :class:`moabb.datasets.Yilmaz2024` (8 subjects, binary tap vs swipe motor-imagery brain-mobile-phone interaction, 13-channel at 128 Hz), :class:`moabb.datasets.Leeuwis2021` (55 subjects, binary left/right-hand motor imagery with psychological and cognitive profiling, 16-channel at 250 Hz), :class:`moabb.datasets.Shin2022` (10 subjects, binary closed-loop one-dimensional cursor-control left/right-hand motor imagery, 16-channel dry-electrode at 250 Hz), :class:`moabb.datasets.WRCC2023_MI_A`, :class:`moabb.datasets.WRCC2023_MI_B` and :class:`moabb.datasets.WRCC2023_MI_C` (9 / 9 / 8 subjects, 3-class left-hand / right-hand / feet motor imagery from the WRCC2023 BCI competition, 59-channel at 1000 Hz), and :class:`moabb.datasets.ZjuMI2025` (15 subjects, 4-class left-hand / right-hand / tongue / feet motor imagery across two sessions, 62-channel at 256 Hz) (by `Bruno Aristimunha`_)
- Add 4 new clinical motor-imagery dataset adapters: :class:`moabb.datasets.Wang2026` (24 subjects, longitudinal binary sixth-finger and affected-hand motor imagery of stroke patients, 60-channel at 250 Hz), :class:`moabb.datasets.Jia2019` (15 stroke patients, binary left/right motor imagery, 63-channel at 512 Hz), :class:`moabb.datasets.Patel2025` (8 ALS patients, 3-class longitudinal motor imagery, 22-channel at 256 Hz), and :class:`moabb.datasets.Perdikis2018` (2 tetraplegic users, 3-class Cybathlon BCI-race motor imagery from a longitudinal mutual-learning study, 16-channel at 512 Hz) (by `Bruno Aristimunha`_)
- Add 4 new motor-imagery dataset adapters: :class:`moabb.datasets.MartinezPeon2024` (10 subjects, 3-class graded kinesthetic motor-imagery levels, 14-channel at 128 Hz), :class:`moabb.datasets.Russo2024` (18 subjects including multiple-sclerosis patients, 10-class imagined movements under a go/no-go protocol, 24-channel at 2048 Hz), :class:`moabb.datasets.InMID2024` (23 Indonesian student subjects, 3-class motor imagery, 14-channel at 128 Hz), and :class:`moabb.datasets.Vasilyev2021` (7 subjects, binary multiday motor imagery with and without feedback, 30-channel at 500 Hz) (by `Bruno Aristimunha`_)
- Add 3 new motor-imagery dataset adapters: :class:`moabb.datasets.Vagaja2023` (26 subjects, binary motor imagery with embodiment and MI-BCI training in virtual reality, 32-channel at 500 Hz), :class:`moabb.datasets.Ding2025` (21 subjects, 4-class individual-finger motor imagery for real-time robotic-hand control, 128-channel at 1024 Hz), and :class:`moabb.datasets.Li2026` (5 subjects, 10-class motor imagery covering diverse cognitive states, 64-channel at 1000 Hz) (by `Bruno Aristimunha`_)
- Add 3 new motor-imagery dataset adapters: :class:`moabb.datasets.Han2026` (35 subjects, binary basketball motor-observation vs motor-imagery of dribbling, 64-channel at 1000 Hz, OpenNeuro ``ds007327``), :class:`moabb.datasets.Batista2022` (20 subjects, binary motor-imagery BCI training with virtual reality and haptic feedback, 32-channel at 500 Hz), and :class:`moabb.datasets.InEarMI2026` (26 subjects, binary left- vs right-hand motor imagery from a 7-channel in-ear EEG montage at 500 Hz) (by `Bruno Aristimunha`_)
- Add 4 new motor-imagery dataset adapters: :class:`moabb.datasets.Pan2023` (14 subjects, binary cross-session left/right-hand motor imagery across two sessions, 28-channel at 250 Hz), :class:`moabb.datasets.PardoGarcia2026` (18 participants: 10 chronic-stroke patients and 8 healthy controls; binary image-cued hand motor imagery — precision pinch vs closed fist — exposed over a conservative 0-1.5 s pre-execution interval, 63-channel at 1000 Hz), :class:`moabb.datasets.Kodera2023` (29 subjects, binary left/right-hand motor imagery, 9-channel normalized from 9/16-channel source recordings at 500/1000 Hz), and :class:`moabb.datasets.Sun2026` (14 subjects, 4-class cross-environment multi-paradigm motor imagery, 64-channel at 1000 Hz, OpenNeuro ``ds007221``) (by `Bruno Aristimunha`_)
- Add 5 new open-access motor-imagery dataset adapters mirrored on OpenNeuro: :class:`moabb.datasets.Cai2026` (IACKD; 15 subjects, binary intention-action-conflict motor imagery with synchronized hand kinematics, 26-channel EEG at 1024 Hz, OpenNeuro ``ds006840``), :class:`moabb.datasets.Peterson2022` (10 subjects, binary motor imagery on a low-cost consumer-grade BCI, 15-channel at 125 Hz, OpenNeuro ``ds003810``), :class:`moabb.datasets.Lee2022` (7 orthopedic-impairment patients, 4-class multimodal EEG-fNIRS motor imagery, 18-channel at 500 Hz, OpenNeuro ``ds004022``), :class:`moabb.datasets.LioiXP1` (10 subjects, binary motor-imagery neurofeedback from a simultaneous EEG-fMRI protocol, 63-channel, OpenNeuro ``ds002336``), and :class:`moabb.datasets.Lioi2020` (17 subjects, binary EEG-fMRI neurofeedback motor imagery, 63-channel, OpenNeuro ``ds002338``) (by `Bruno Aristimunha`_)
- Add 5 new motor-execution and reach-to-grasp dataset adapters: :class:`moabb.datasets.MOVING2024` (11 subjects, 4-class hand movements with virtual-glove hand tracking, 32-channel at 500 Hz), :class:`moabb.datasets.DFKI2023` (8 subjects, binary unilateral/bilateral movement execution for exoskeleton rehabilitation, 64-channel at 500 Hz), :class:`moabb.datasets.Lomele2026` (14 subjects, 3-class object prehension with a sensorized grasping box, high-density 64-channel EEG and multi-muscle EMG), :class:`moabb.datasets.HYGRIP2020` (14 subjects, binary bimanual grip-force control with synchronized fNIRS/EEG/EMG/force/breathing, 24-channel at 1000 Hz), and :class:`moabb.datasets.Farabbi2020` (12 subjects, binary motor imagery during robot-arm control, 32-channel at 250 Hz) (by `Bruno Aristimunha`_)
- Add 6 new rehabilitation and clinical motor-imagery dataset adapters: :class:`moabb.datasets.Lu2026` (HS-Stroke; 14 chronic-stroke patients, binary upper-limb motor imagery across 20 sessions, 11-channel at 256 Hz), :class:`moabb.datasets.Rehab2025OpenBCI` (8 subjects, binary motor imagery for neurorehabilitation recorded with OpenBCI, 8-channel at 1000 Hz), :class:`moabb.datasets.PoloHortiguela2025` (6 subjects, binary dorsiflexion/plantar-flexion movements, 28-channel at 250 Hz), :class:`moabb.datasets.SitStand2026` (22 subjects, binary sit-to-stand and stand-to-sit motor imagery and execution, 60-channel at 1200 Hz), :class:`moabb.datasets.SpinalStim2025` (27 able-bodied and spinal-cord-injury subjects, binary longitudinal MI-BCI training with transcutaneous spinal stimulation, 32-channel at 512 Hz), and :class:`moabb.datasets.Medvedeva2026` (16 post-stroke patients, binary intact/paretic hand movements from a multisession fNIRS-EEG programme, 8-channel at 500 Hz) (by `Bruno Aristimunha`_)
- Add 4 new motor-imagery dataset adapters: :class:`moabb.datasets.OpenViBE` (14 subjects, binary Graz-protocol motor imagery from the OpenViBE example recordings, 11-channel at 512 Hz), :class:`moabb.datasets.Pan2025` (10 subjects, binary cross-session motor imagery, 28-channel at 250 Hz), :class:`moabb.datasets.neuroTUMBCI` (2 pilots, 3-class directional motor imagery for the 2024 Cybathlon, 24-channel mobile EEG at 250 Hz), and :class:`moabb.datasets.Wirawan2024` (MIMED; 23 unique imagery recordings under 30 release labels, 3-class motor imagery, 14-channel at 128 Hz) (by `Bruno Aristimunha`_)
- Add 3 new motor-imagery/motor-execution dataset adapters: :class:`moabb.datasets.Thapa2025` (23 subjects, self-paced freewill 4-class reach-and-grasp motor execution toward one of four cups, 31-channel BrainVision BIDS archive on Figshare), :class:`moabb.datasets.Damm2026` (20 subjects, 5-class finger motor imagery — thumb/index/middle/ring/pinky — cued in random and sequential block orders, 62-channel g.tec EEG at 512 Hz, OpenNeuro ``ds008446``), and :class:`moabb.datasets.Daly2020` (18 subjects, binary kinesthetic motor imagery — right-hand ball-squeeze vs relax — from the BCMI-MIdAS tempo-based brain-computer music interface, 19-channel EEG at 1000 Hz, OpenNeuro ``ds002720``) (by `Bruno Aristimunha`_)
- Add 7 new motor-imagery/motor-execution dataset adapters: :class:`moabb.datasets.Garro2025` (NeBULA, 39 subjects, 3-class right-arm reaching under three robotic assistance levels, 127-channel actiCHamp EEG + surface EMG), :class:`moabb.datasets.PerezBlanco2026` (45 subjects, 4-class wrist-pointing motor execution with a rehabilitation exoskeleton, 8-channel EEG + EMG), :class:`moabb.datasets.Iwama2023` (30 subjects, high-density 128-channel SMR-neurofeedback BCI, right-hand motor imagery vs rest across up to 16 sessions, OpenNeuro ``ds004444``), :class:`moabb.datasets.KMIHandGrip2025` (50 subjects, 4-class graded-force kinesthetic motor imagery of hand-grip), :class:`moabb.datasets.MILimbEEG` (60 subjects, 8-class upper- and lower-limb motor imagery), :class:`moabb.datasets.MIND2026` (30 subjects, 4-class directional motor imagery with simultaneous fNIRS recording; EEG modality only), and :class:`moabb.datasets.Song2026` (45 chronic-stroke patients, 4-class motor-attempt decoding of the affected upper limb) (by `Bruno Aristimunha`_)
...
- Remove :class:`moabb.datasets.BCIComp2020IS` (BCI Competition 2020 Track 3 Imagined Speech): the released ``.mat`` files publish trials in a scrambled order without the original time information, so per-trial temporal context is lost and the dataset cannot be evaluated reliably — any reported accuracy on random splits is likely inflated by residual intra-block leakage. The authors could not be reached to recover the trial timing, so the loader is removed entirely (by `Bruno Aristimunha`_)
...
Bugs
~~~~
- Exclude the duplicate participant labels P24-P30 from :class:`moabb.datasets.Wirawan2024`: their imagery arrays repeat P17-P23 exactly across every scenario and repetition, which could leak identical waveforms across subject-wise train/test folds. The adapter now exposes the 23 unique recordings, and :class:`moabb.datasets.InMID2024` documents that its imagery session reuses the same MIMED P01-P23 cohort (by `Bruno Aristimunha`_)
