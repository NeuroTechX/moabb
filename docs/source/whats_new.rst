...
- Preserve :class:`moabb.datasets.BNCI2014_001` source artifact flags with optional ``artifact_handling`` annotation modes, and expose ``reject_by_annotation`` in motor-imagery epoching so users can choose whether ``BAD_artifact`` trials are dropped during epoch generation (by `copilot-swe-agent`_).
