...
- Fix :class:`moabb.datasets.AguileraRodriguez2025` gamified sessions failing to load: the recorder wrote a non-conforming XDF footer (float ``sample_count``), which makes ``pyxdf`` 1.17.5's corrupted-offset check raise ``ValueError`` for every subject. The XDF data is now read by a built-in minimal reader (``moabb.datasets._xdf``) that ignores the footer entirely, so the dataset no longer needs the optional ``pyxdf`` dependency at all. Validated against ``pyxdf`` on the real recordings: bit-identical EEG arrays, identical marker labels, marker onsets within 0.25 ms (a 500 Hz sample is 2 ms) (by `Bruno Aristimunha`_)
