...
Enhancements
~~~~~~~~~~~~
- Shuffle trials with a fixed seed in :class:`moabb.datasets.BCIComp2020IS`. The original recording uses a block-like design (trials of the same class recorded contiguously) and the released ``.mat`` files preserve that ordering; re-stitching trials into a Raw object in their on-disk order leaks intra-block temporal correlations into random-split evaluations and inflates scores. The loader now permutes trials at load time using an internal ``(subject, run_idx)``-derived seed (not user-configurable, so scores stay comparable across callers) (by `Bruno Aristimunha`_)
