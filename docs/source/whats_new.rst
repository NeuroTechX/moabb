...
- Cache Figshare's article-level file listing in :func:`moabb.datasets.download.fs_get_file_list` (process-level ``lru_cache`` keyed by ``(article_id, version)``) and persist it on disk next to the data for the MAMEM SSVEP loaders (:class:`moabb.datasets.MAMEM1`/``MAMEM2``/``MAMEM3``). Earlier code re-issued the public Figshare API call once per subject inside ``data_path`` *and* ``_get_single_subject_data``, which 403-rate-limited from datacenter IPs whenever a user iterated over subjects. Cached listings let subsequent runs load already-downloaded data without ever contacting Figshare; ``force_update=True`` bypasses both layers (by `Bruno Aristimunha`_).
