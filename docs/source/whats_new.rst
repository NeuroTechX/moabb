...
- Bump minimum ``mne-bids`` to ``0.19`` for its cross-process file lock (``mne_bids._fileio._open_lock``), used to keep parallel BIDS caching (``get_data(n_jobs>1)``) consistent (:gh:`1124` by `Bruno Aristimunha`_)
