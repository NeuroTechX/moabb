...
Enhancements
~~~~~~~~~~~~
- Add four motor-imagery datasets from the Wang et al. 2026 sensory-guided joint-learning release: :class:`moabb.datasets.Wang2026JointLearning` (15 subjects), :class:`moabb.datasets.Wang2026TactileControl` (8), :class:`moabb.datasets.Wang2026EEGNetControl` (8) and :class:`moabb.datasets.Wang2026BCI2000Control` (8). Longitudinal online 1D and 2D cursor control on a 62-channel Compumedics Neuroscan Quik-Cap at 1000 Hz, four classes (left hand, right hand, both hands, rest), four to eight regular sessions per subject plus an initial BCI2000 baseline run. The four experimental arms received different training interventions, so they are exposed as separate datasets rather than one pooled cohort and subjects are never mixed across arms. Each arm is published as a single 8.7-25.3 GB archive with no per-subject files, so one subject is extracted from the remote ZIP over HTTP range requests instead of downloading the whole arm (:gh:`1126` by `Paul-Adrien Graignic`_ and `Bruno Aristimunha`_)
