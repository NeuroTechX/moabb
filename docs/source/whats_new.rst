...
- Fix EEG layout corruption in :class:`moabb.datasets.BNCI2020_002` (``_convert_attention_shift``): ``bciexp.data`` is F-contiguous out of ``scipy.io.loadmat``, so the previous default-order ``reshape(n_channels, -1)`` produced a trial-fastest interleaved layout (``flat[c, k] = data[c, k // n_trials, k % n_trials]``) that disagreed with the per-trial stim markers. Every per-stimulus epoch was therefore sampled from the wrong trial and the canonical N2pc analysis sat at chance. The reshape now does ``transpose(0, 2, 1).reshape(...)`` so that ``flat[c, t * n_samples + s] == data[c, s, t]`` (by `Bruno Aristimunha`_).
- Fix ``stim_trial`` content in :class:`moabb.datasets.MartinezCagigal2023Checker` and :class:`moabb.datasets.MartinezCagigal2023Pary`: the channel was carrying the per-recording trial index ``0..n_trials-1`` instead of the attended command id, so the same marker value pointed at different attended symbols across recordings and broke multiclass classification. The marker is now the attended command id (resolved from ``cvep_data["command_idx"]`` for train and from ``commands_info`` for test), aligning with the convention used by :class:`moabb.datasets.castillos2023.BaseCastillos2023` and the Thielen c-VEP datasets. The ``_trial_meta`` annotation extras gain a ``command_id`` key alongside ``trial_id`` so trial identity *and* attended symbol both survive BIDS export (by `Bruno Aristimunha`_).
- Cache Figshare's article-level file listing in :func:`moabb.datasets.download.fs_get_file_list` (process-level ``lru_cache`` keyed by ``(article_id, version)``) and persist it on disk next to the data for the MAMEM SSVEP loaders (:class:`moabb.datasets.MAMEM1`/``MAMEM2``/``MAMEM3``). Earlier code re-issued the public Figshare API call once per subject inside ``data_path`` *and* ``_get_single_subject_data``, which 403-rate-limited from datacenter IPs whenever a user iterated over subjects. With this change, **once the dataset has been downloaded the loader never contacts Figshare again** — the on-disk JSON cache (``figshare_filelist_<id>.json``) is read instead, so loading any subject from an already-downloaded MAMEM dataset is a pure local-disk operation. ``force_update=True`` bypasses both layers when a republished article version genuinely needs to be picked up (by `Bruno Aristimunha`_).
