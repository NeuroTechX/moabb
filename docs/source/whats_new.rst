...
Bugs
~~~~
- Store :class:`moabb.datasets.ErpCore2021` as the single combined BIDS dataset it is, with components separated by the ``task-`` entity in one shared ``MNE-erpcore2021-data`` root, instead of seven standalone per-component BIDS datasets. Pre-existing downloads in the legacy separated layout are still read without re-fetching (:gh:`1146` by `Bruno Aristimunha`_)
