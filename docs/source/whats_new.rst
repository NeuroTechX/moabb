...
- Fix :class:`moabb.evaluations.splitters.WithinSessionSplitter` and :class:`moabb.evaluations.splitters.WithinSubjectSplitter` overwriting an explicit ``n_splits`` (and ``shuffle`` / ``random_state``) passed through ``cv_kwargs`` with the ``n_folds`` default. Caller-provided values now take precedence, so a single holdout split can be requested directly via ``cv_class=StratifiedShuffleSplit, n_splits=1`` (:gh:`1106` by `Bruno Aristimunha`_).
Code health
~~~~~~~~~~~
