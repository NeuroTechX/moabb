...
Bugs
~~~~
- Fix :class:`moabb.datasets.BNCI2014_001` descriptive ``METADATA`` that had several fields copied from BCI Competition IV Data set 1: correct ``n_subjects`` (4 → 9), ``n_classes`` (2 → 4), ``class_labels`` and ``events`` (now ``left_hand`` / ``right_hand`` / ``feet`` / ``tongue``), ``synchronicity`` (asynchronous → synchronous), and the dataset ``description`` so they match the dataset's own constructor and docstring (by `YG-paaleee`_)
