...
def estimator_accepts_argument(
    estimator: Any,
    method_name: str,
    arg_name: str,
) -> bool:
...
def _fit_transform_step_with_metadata(
    ...
    if (
        X_target_unlabeled is not None
        and estimator_accepts_argument(step, "fit", "X_target_unlabeled")
    ):
        fit_kwargs["X_target_unlabeled"] = X_target_unlabeled

    if (
        X_target_labeled is not None
        and estimator_accepts_argument(step, "fit", "X_target_labeled")
    ):
        fit_kwargs["X_target_labeled"] = X_target_labeled

    if (
        y_target_labeled is not None
        and estimator_accepts_argument(step, "fit", "y_target_labeled")
    ):
...
def fit_pipeline_with_subject_metadata(
    ...
    if not isinstance(estimator, Pipeline):
        ...
        if (
            X_target_unlabeled is not None
            and estimator_accepts_argument(estimator, "fit", "X_target_unlabeled")
        ):
            fit_kwargs["X_target_unlabeled"] = X_target_unlabeled

        if (
            X_target_labeled is not None
            and estimator_accepts_argument(estimator, "fit", "X_target_labeled")
        ):
            fit_kwargs["X_target_labeled"] = X_target_labeled

        if (
            y_target_labeled is not None
            and estimator_accepts_argument(estimator, "fit", "y_target_labeled")
        ):
            fit_kwargs["y_target_labeled"] = y_target_labeled
    ...
    if (
        Xt_target_unlabeled is not None
        and estimator_accepts_argument(final_step, "fit", "X_target_unlabeled")
    ):
        final_fit_kwargs["X_target_unlabeled"] = Xt_target_unlabeled

    if (
        Xt_target_labeled is not None
        and estimator_accepts_argument(final_step, "fit", "X_target_labeled")
    ):
        final_fit_kwargs["X_target_labeled"] = Xt_target_labeled

    if (
        y_target_labeled is not None
        and estimator_accepts_argument(final_step, "fit", "y_target_labeled")
    ):
...
class TrialwisePredictWrapper(ClassifierMixin, BaseEstimator):
    ...
    def predict_proba(self, X):
        ...
        rows = [
            self._first_row(
                self.fitted_estimator.predict_proba(self._slice_one(X, i))
            )
            for i in range(len(X))
        ]
...
class CrossSubjectTargetAwareEvaluation(CrossSubjectEvaluation):
    ...
    def __init__(
        self,
        *args,
        cs_mode: CsMode = CsMode.HOS_SOURCE_ONLY_BLOCKWISE,
        **kwargs,
    ):
        ...
        if not isinstance(cs_mode, CsMode):
            raise ValueError(
                "cs_mode must be an instance of CsMode. "
                f"Got {cs_mode!r}."
            )
    ...
    def _build_task_list(
        ...
        for cv_ind, (train_idx, test_idx) in enumerate(splitter.split(y, metadata)):
            ...
            target_adapt_idx, eval_idx = split_target_for_cs_mode(
                test_idx,
                self.cs_mode,
            )
