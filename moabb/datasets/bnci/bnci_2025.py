    ...
    **Event Codes**
    ...
    METADATA = DatasetMetadata(
        ...
        experiment=ExperimentMetadata(
            ...
            trial_duration=None,
            study_design="Four-direction center-out reaching task with varying speeds (quick/slow) and distances (near/far) following visual cue, self-paced execution with eye fixation on cue",
        ...
        documentation=DocumentationMetadata(
            ...
            funding=[
                "Royal Thai Government (scholar funding for N.S.)",
                "BioTechMed Graz",
            ],
            keywords=[
        ...
        preprocessing=PreprocessingMetadata(
            ...
            preprocessing_steps=[
                ...
                "eye artifact correction via SGEYESUB",
                "ICA with FastICA algorithm",
                "IC artifact removal",
            ...
            artifact_methods=[
                "ICA",
                "SGEYESUB (Sparse Generalized Eye Artifact Subspace Subtraction)",
                "IClabel plugin",
            ],
            re_reference="common average",
            downsampled_to_hz=200.0,
            epoch_window=[-3.0, 4.0],
        ...
        performance={
            ...
            "direction_accuracy_mon_percent": 42.42,
            "speed_accuracy_cstp_percent": 66.03,
            "speed_accuracy_mon_percent": 70.49,
            "distance_accuracy_cstp_percent": 60.83,
            "distance_accuracy_mon_percent": 55.41,
            "quick_direction_accuracy_cstp_percent": 44.12,
            "quick_direction_accuracy_mon_percent": 49.67,
            "slow_direction_accuracy_cstp_percent": 37.42,
            "slow_direction_accuracy_mon_percent": 35.89,
        },
        ...
        data_structure=DataStructureMetadata(
            n_trials=960,
            trials_context="per_participant (before rejection)",
            n_trials_per_class={"up": 240, "down": 240, "left": 240, "right": 240},
            n_blocks=10,
            block_duration_s=1200.0,
        ),
        paradigm_specific=ParadigmSpecificMetadata(
            detected_paradigm="motor_imagery",
            imagery_tasks=["right_hand_reaching"],
            cue_duration_s=None,
    ...
    **Data Organization**
    ...
    METADATA = DatasetMetadata(
        acquisition=AcquisitionMetadata(
            ...
            filters="anti-aliasing 25 Hz, notch 50 Hz",
            sensors=[
                "AF3",
                "AF4",
                "AF7",
        ...
        participants=ParticipantMetadata(
            n_subjects=10,
            health_status="Healthy (able-bodied participants) + 1 SCI participant",
        ...
        experiment=ExperimentMetadata(
            ...
            class_labels=None,
            trial_duration=23.0,
            study_design="Attempted movement paradigm: participants instructed to attempt lower arm movement as if wielding a computer mouse while arm was strapped to armrest. Two task types: snakeruns (target tracking) and freeruns (self-paced shape tracing). Offline calibration followed by online feedback in 50% and 100% EEG feedback conditions.",
            feedback_type="visual (green dot showing EEG-decoded trajectory position)",
            stimulus_type="visual targets (white snake/shapes on black screen)",
            stimulus_modalities=["visual"],
            primary_modality="visual",
        ...
        documentation=DocumentationMetadata(
            ...
            contact_info=["gernot.mueller@tugraz.at"],
            funding=[
                "European Research Council ERC-CoG 2015 681231 'Feel Your Reach'",
                "NTU-TUG joint PhD program",
            ],
            institution_address="Stremayrgasse 16/IV, 8010 Graz, Austria",
            ethics_approval=["Medical University of Graz, votum number 32–583 ex 19/20"],
            keywords=[
                "electroencephalography",
                "trajectory decoding",
                "learning effects",
        ...
        preprocessing=PreprocessingMetadata(
            ...
            preprocessing_applied=True,
            preprocessing_steps=[
                "anti-aliasing filter (25 Hz)",
                "notch filter (50 Hz)",
                "downsampling to 100 Hz",
                "bad channel interpolation",
                "eye artifact subtraction (SGEYESUB algorithm)",
                "removal of frontal (AF) row channels",
                "high-pass filter (0.18 Hz)",
                "common average re-reference",
                "pops and drifts attenuation (HEAR algorithm)",
            ...
            artifact_methods=[
                "SGEYESUB (eye artifact subtraction)",
                "HEAR (pops and drifts removal)",
            ],
            re_reference="common average reference",
            downsampled_to_hz=20.0,
        ),
        signal_processing=SignalProcessingMetadata(
            classifiers=["PLS regression with UKF smoothing"],
            feature_extraction=[
                "Temporal features (7 time points × 55 channels = 385 features)",
                "sLORETA (source localization)",
            ],
            spatial_filters=["Minimum norm imaging"],
        ),
        cross_validation=CrossValidationMetadata(
            cv_method="across-session",
            evaluation_type=["within-subject", "learning effects over sessions"],
        ),
        performance={
            "normalized_correlation_mean": 0.31,
            "normalized_correlation_std": 0.02,
            "correlation_range_rc": "0.4-0.5",
            "NRMSE_calibration": 0.10,
            "NRMSE_100%_feedback": 0.12,
        },
        bci_application=BCIApplicationMetadata(
            applications=[
                "neuroprosthesis",
                "robotic arm control",
                "upper limb restoration",
            ],
            environment="laboratory",
            online_feedback=True,
        ),
        paradigm_specific=ParadigmSpecificMetadata(
            detected_paradigm="motor attempt",
            imagery_tasks=["attempted arm/hand movement (2D continuous trajectory)"],
        ),
        data_structure=DataStructureMetadata(
            n_trials={
                "calibration_eyeruns": 38,
                "calibration_snakeruns": 48,
                "50%_EEG_feedback_snakeruns": 36,
                "100%_EEG_feedback_snakeruns": 36,
                "freeruns": 9,
            },
            trials_context="per_paradigm_type",
        ),
