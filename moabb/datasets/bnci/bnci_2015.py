    ...
    **Recording Details**
    ...
    METADATA = DatasetMetadata(
        ...
        documentation=DocumentationMetadata(
            ...
            readme='Conflicting metadata across sources:\\n- experimental_design.task_description: main=Two-class motor imagery: sustained right hand movement imagery (palmar grip) versus both feet movement imagery (plantar ...; description=to relax and to perform sustained, kinesthetic movement im- agery ([5]) during the complete activity period of the prese... (kept description)\\n- experimental_design.trial_structure: main=3s reference period, audible and visual cue at second 3, cue displayed 3-4.25s, activity/feedback period 4-8s, random 2-...; description=3s reference period, audible cue at 3s, visual cue arrow from 3-4.25s, activity/feedback period from 4-8s, 2-3s random p... (kept description)\\n- paradigm_specific.detected_paradigm: main=mi; description=motor_imagery (kept description)\\n- participants.health_status: main=healthy; description=ALS (kept description)\\n- participants.age_mean: main=25; description=24.8 (kept description)\\n- participants.age_std: main=3; description=3.0 (kept description)\\n- preprocessing.preprocessing_literal: main=3, MAY 2012 313 Autocalibration and Recurrent Adaptation: Towards a Plug and Play Online ERD-BCI Josef Faller, Carmen Vi...; description=During operation, the system recurrently reselects only one out of six predefined logarithmic band- power features (10 t... (kept description)\\n- preprocessing.preprocessing_details.artifact_methods: main=["trial rejection", "ICA"]; description=["ICA"] (kept description)\\n- preprocessing.data_state: main=online processed; description=filtered (kept description)\\n- preprocessing.claude_steps: main=["bandpass filter", "notch filter", "Laplacian spatial filtering", "outlier rejection"]; description=["bandpass filter", "notch filter"] (kept description)\\n- ... and 5 more conflicts',
        ),
        tags=Tags(
            pathology=["Healthy"],
            modality=["Motor"],
    ...
    **Dataset Description**
    ...
    METADATA = DatasetMetadata(
        ...
        documentation=DocumentationMetadata(
            ...
            readme='Conflicting metadata across sources:\\n- experimental_design.paradigm_description: main=MI; description=Motor imagery (kept description)\\n- experimental_design.task_description: main=Five mental tasks: word association, mental subtraction, spatial navigation, right hand motor imagery, both feet motor i...; description=avoid movements during the imagery period, and to move and blink during the ITI. (kept description)\\n- experimental_design.trial_structure: main=fixation cross at t=0s, beep at t=3s, cue from t=3s to t=4.25s, imagery for 7s, end beep at t=10s; description=Cross fixation at t=0s, beep at t=3s, cue presentation from t=3s to t=4.25s, imagery period, second beep at t=10s indica... (kept description)\\n- participants.health_status: main=healthy; description=SCI and stroke (kept description)\\n- preprocessing.data_state: main=epoched; description=raw filtered (kept description)\\n- preprocessing.claude_steps: main=["visual scoring by expert", "artifact rejection", "noisy channel removal", "common spatial patterns (CSP) filtering"]; description=["bandpass filter", "notch filter"] (kept description)\\n- preprocessing_claude.preprocessing_stage: main=offline; description=online (kept description)\\n- preprocessing_claude.preprocessing_steps: main=["visual scoring by expert", "artifact rejection", "noisy channel removal", "common spatial patterns (CSP) filtering"]; description=["bandpass filter", "notch filter"] (kept description)\\n- preprocessing_claude.filter_details.bandpass: main=[8, 30]; description=[0.5, 100] (kept description)\\n- preprocessing_claude.data_state: main=epoched; description=raw filtered (kept description)\\n- ... and 1 more conflicts',
        ),
        tags=Tags(
            pathology=["Healthy"],
            modality=["Motor"],
    ...
    **Dataset Description**
    ...
    METADATA = DatasetMetadata(
        ...
        documentation=DocumentationMetadata(
            ...
            funding=["Grant Nos s"],
            readme='Conflicting metadata across sources:\\n- data_structure.n_runs: report=8; paper=10 (kept paper)\\n- experimental_design.task_description: report=Subjects listened to polyphonic music clips featuring three instruments. They were cued to attend to one particular inst...; paper=Participants attended to one of three instruments in polyphonic music clips and counted the number of deviants in the cu... (kept paper)\\n- experimental_design.trial_structure: report=Each run features a particular experimental condition and consists of multiple music clips. Each music clip is preceded ...; paper=Visual cue indicating to-be-attended instrument, followed by standard and deviant stimulus playback, then 2s fixation, t... (kept paper)\\n- preprocessing.preprocessing_literal: report=3 Data For details on data pre-processing, refer to the papers in the reference list. The data has been pre-processed wi...; paper=Data analysis For offline analysis, the data was downsampled to 250 Hz and lowpass filtered using a Chebyshev filter (wi... (kept paper)\\n- preprocessing.data_state: report=continuous; paper=epoched (kept paper)\\n- preprocessing.claude_steps: report=Preprocessed with BBCI Matlab toolbox; paper=["downsampling", "lowpass filtering", "epoching", "baseline correction", "artifact rejection"] (kept paper)\\n- preprocessing_claude.preprocessing_stage: report=preprocessed; paper=offline (kept paper)\\n- preprocessing_claude.preprocessing_steps: report=Preprocessed with BBCI Matlab toolbox; paper=["downsampling", "lowpass filtering", "epoching", "baseline correction", "artifact rejection"] (kept paper)\\n- preprocessing_claude.data_state: report=continuous; paper=epoched (kept paper)\\n- preprocessing_claude.notes: report=Data can be transformed into FieldTrip, EEGLab formats manually. Contains event markers for attended/unattended standard...; paper=Data downsampled to 250 Hz; Chebyshev lowpass filter with passband 42 Hz and stopband 49 Hz; prestimulus interval used f... (kept paper)',
        ),
        tags=Tags(
            pathology=["Healthy"],
            modality=["Visual"],
    ...
    **Event Codes**
    ...
    METADATA = DatasetMetadata(
        ...
        documentation=DocumentationMetadata(
            ...
            repository="GitHub",
            data_url="https://github.com/bbci/bbci_public/blob/master/doc/index.markdown",
            readme='Conflicting metadata across sources:\\n- experimental_design.mode: main=both; description=online (kept description)\\n- experimental_design.task_description: main=sit still, relax their muscles and try to minimize eye movements during the course of a trial.; description=Visual speller using geometric shapes with unique colors presented centrally in sequential fashion. Two-level selection:... (kept description)\\n- experimental_design.trial_structure: main=5 s countdown, then intensification phase (~30 s) with 10 sequences of 6 intensifications per stage (2 stages per symbol...; description=Countdown showing all letter groups and symbols at different spatial locations, then sequential central presentation of ... (kept description)\\n- preprocessing.preprocessing_literal: main=For offline analysis, electrodes were re-referenced to linked mastoids. Data analysis For offline ERP analysis, the data...; description=2 Data For details on data pre-processing, refer to the papers in the reference list. The data has been pre-processed wi... (kept description)\\n- preprocessing.data_state: main=preprocessed for ERP analysis; raw (downsampled to 100 Hz, no software filter) for classification; description=continuous EEG with event markers (kept description)\\n- preprocessing.claude_steps: main=["downsampling", "lowpass filtering", "epoching", "baseline correction"]; description=["BBCI Matlab toolbox preprocessing"] (kept description)\\n- preprocessing_claude.preprocessing_stage: main=offline; description=pre-processed (kept description)\\n- preprocessing_claude.preprocessing_steps: main=["downsampling", "lowpass filtering", "epoching", "baseline correction"]; description=["BBCI Matlab toolbox preprocessing"] (kept description)\\n- preprocessing_claude.artifact_handling: main=trials with eye movements detected by eyetracker were aborted and discarded; description=eye movement monitoring with eyetracker, trials restarted if subject did not fixate (marked as invalid) (kept description)\\n- preprocessing_claude.data_state: main=preprocessed for ERP analysis; raw (downsampled to 100 Hz, no software filter) for classification; description=continuous EEG with event markers (kept description)\\n- ... and 2 more conflicts',
        ),
        tags=Tags(
            pathology=["Healthy"],
            modality=["Visual"],
    ...
    **Dataset Description**
    ...
    METADATA = DatasetMetadata(
        ...
        documentation=DocumentationMetadata(
            ...
            readme='Conflicting metadata across sources:\\n- data_structure.n_trials: main=50; paper=64 (kept paper)\\n- experimental_design.stimulus_type: main=cursor_feedback; paper=oddball (kept paper)\\n- experimental_design.paradigm_type: main=error-related potentials (ErrP); paper=error-related potential (ErrP) monitoring (kept paper)\\n- experimental_design.task_description: main=Subjects monitored an autonomous cursor moving toward a target location. The cursor moved correctly 80% of the time and ...; paper=Subjects monitored an autonomous cursor moving toward a target on screen without providing control commands. The cursor ... (kept paper)\\n- experimental_design.trial_structure: main=Cursor moves horizontally toward target at each time step. Once target reached, new target drawn within 3 positions of c...; paper=At each time step, cursor moves horizontally toward target location. Target appears at left (blue) or right (red) of cur... (kept paper)\\n- participants.age_mean: main=27.83; paper=27.5 (kept paper)\\n- participants.age_std: main=2.23; paper=4 (kept paper)\\n- preprocessing.preprocessing_details.artifact_methods: main=["ICA"]; paper=["EOG correction", "ICA"] (kept paper)\\n- preprocessing.claude_preprocessing_applied: main=False; paper=True (kept paper)\\n- preprocessing.data_state: main=raw; paper=epoched (kept paper)\\n- ... and 4 more conflicts',
        ),
        tags=Tags(
            pathology=["Healthy"],
            modality=["Visual"],
