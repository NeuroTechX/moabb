...
def _resolve_subject_age(participants, subject_idx, raw=None):
    ...
    if raw is not None and hasattr(raw, "_moabb_subject_age"):
        raw_age = raw._moabb_subject_age
...
def _build_dataset_description_kwargs(dataset):
    ...
    kwargs = {
        ...
        "source_datasets": [{"DOI": dataset.doi or "n/a"}],
    }
...
def _update_participants_tsv(root, subject, metadata, raw=None):
    ...
    if "age" not in sidecar:
        ...
        sidecar["age"] = {"Description": age_desc, "Units": "years"}
        updated = True
    if "group" not in sidecar:
        sidecar["group"] = {
            "Description": "Group the participant belonged to (e.g., healthy, patients)"
        }
        updated = True
    ...
    if "species" not in sidecar:
        sidecar["species"] = {"Description": "Species of the participant (binomial name)"}
        updated = True
...
def _build_readme(dataset):
    ...
    # ── Performance ──
    if perf:
        _perf_units = {"accuracy_percent": "%", "itr_bits_per_min": " bits/min"}
    ...
    # ── Footer ──
    lines.append("---")
    lines.append(f"Generated by MOABB {moabb.__version__} (Mother of All BCI Benchmarks)")
...
class BIDSInterfaceBase(abc.ABC):
    ...
    def erase(self):
        ...
        sessions = {p.session for p in paths}
    ...
    def load(self, preload=False):
        ...
        a nested dictionary with the following structure::
            ...
            sessions_data = {"session_id": {"run_id": run}}
    ...
    def save(self, sessions_data):
        ...
        lock_data = {"processing_params": str(self.processing_params)}
        ...
        mne_bids.make_dataset_description(
            path=str(self.root), overwrite=True, verbose=self.verbose, **desc_kwargs
        )
...
class BIDSInterfaceRawEDF(BIDSInterfaceBase):
    ...
    def _load_file(self, bids_path, preload):
        raw = mne_bids.read_raw_bids(
            bids_path, extra_params={"preload": preload}, verbose=self.verbose
        )
        return raw
    ...
    def _write_file(self, bids_path, raw):
        ...
        if raw.info.get("subject_info", None) is None:
            ...
            raw.info["subject_info"] = {"his_id": subject_moabb_to_bids(self.subject)}
        ...
        if channels_tsv.fpath.exists():
            ...
            if not channels_json_path.exists():
                ...
                ch_types = {mne.channel_type(raw.info, i) for i in range(n_ch)}
...
class BIDSInterfaceNumpyArray(BIDSInterfaceBase):
    ...
    def _write_file(self, bids_path, obj):
        events_path = bids_path.copy().update(suffix="events", extension=".eve")
