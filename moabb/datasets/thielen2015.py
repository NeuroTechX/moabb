    ...
    **Dataset description**
    ...
    METADATA = DatasetMetadata(
        ...
        experiment=ExperimentMetadata(
            ...
            stimulus_type="pseudo-random noise-code",
        ...
        documentation=DocumentationMetadata(
            ...
            institution_department="Donders Center for Cognition",
            country="NL",
            ...
            ethics_approval=[
                "Ethical Committee of the Faculty of Social Sciences at the Radboud University Nijmegen"
            ],
            keywords=[
        ...
        preprocessing=PreprocessingMetadata(
            ...
            highpass_hz=5,
            lowpass_hz=100,
            bandpass={"band1": [5, 48], "band2": [52, 100]},
            artifact_methods=[],
            re_reference="car",
            downsampled_to_hz=360.0,
        ),
        ...
        performance={
            ...
            "itr_bits_per_min_fixed_length": 38.12,
            "spm_fixed_length": 6.93,
            "accuracy_percent_early_stopping": 86.0,
            "itr_bits_per_min_early_stopping": 48.37,
        ...
        data_structure=DataStructureMetadata(
            n_trials=108,
            trials_context="108 total per subject: 3 fixed-length copy-spelling runs x 36 trials per run, each trial 4.2 seconds (4 code cycles)",
        ),
    ...
    def _get_single_subject_data(self, subject):
        ...
        for i_b in range(NR_RUNS):
            ...
            # Handle ANA and EXG channels
            ana = [f"ANA{1 + i}" for i in range(32)]
            exg = [f"EXG{1 + i}" for i in range(8)]
            if not self.return_all_modalities:
                raw.drop_channels(ana + exg)
            else:
                # Retype non-scalp channels so set_montage (which only touches
                # EEG-typed channels) won't fail looking for their positions.
                ch_type_map = {ch: "misc" for ch in ana + exg if ch in raw.ch_names}
                if ch_type_map:
