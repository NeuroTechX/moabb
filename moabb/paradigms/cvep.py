...
class BaseCVEP(BaseParadigm):
    ...
    filters: list of list (defaults [[1, 40]])
...
class SinglePass(BaseCVEP):
    """Single Bandpass filter c-VEP

    c-VEP paradigm with only one bandpass filter (default 1 to 40 Hz)
    ...
    def __init__(self, fmin=1, fmax=40, **kwargs):
...
class CVEP(SinglePass):
    """c-VEP for Target/NonTarget classification
    ...
    def __init__(self, **kwargs):
        if "events" in kwargs.keys():
            raise (ValueError("c-VEP does not accept events"))
...
class FakeCVEPParadigm(CVEP):
    """Fake c-VEP for Target/NonTarget classification."""
