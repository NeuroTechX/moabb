...
def test_within_session(shuffle, random_state):
    ...
    for  (X_train_t, X_test_t), (train, test) in zip(eval_split_within_session(
            shuffle=shuffle, random_state=random_state),split.split(y, metadata)):
...
def test_is_shuffling():
    ...
    for (train, test), (train_shuffle, test_shuffle) in zip(split.split(y, metadata),
                                                            split_shuffle.split(y, metadata)):
        # Check if the output is the same as the input
