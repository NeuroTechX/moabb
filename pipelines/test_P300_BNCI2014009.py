...
pipelines["ERPCov+MDM"] = make_pipeline(
    ERPCovariances(classes=[labels_dict["Target"]], estimator="scm"), MDM()
)
pipelines["ERPCov+TS+LR_L1"] = make_pipeline(
    ERPCovariances(classes=[labels_dict["Target"]], estimator="scm"),
    TSclassifier(
        metric="riemann", clf=LR(penalty="l1", max_iter=1000, solver="liblinear")
    ),
)
...
pipelines["ERPCov+TS+LR_EN"] = make_pipeline(
    ...
    TSclassifier(
        metric="riemann",
        clf=LR(penalty="elasticnet", max_iter=1000, l1_ratio=0.5, solver="saga"),
    ),
)
...
evaluation = WithinSessionEvaluation(
    paradigm=p300_paradigm, datasets=p300_dataset, suffix="examples", overwrite=True
)
