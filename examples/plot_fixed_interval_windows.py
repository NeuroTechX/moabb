...
# the onset of two consecutive epochs, ``start_offset`` is the offset between each
# run start and their first epoch, and ``stop_offset`` is the offset between each
...
processing = FixedIntervalWindowsProcessing(
    ...
    stride=50,
    start_offset=300,
    stop_offset=900,  # we epoch 10 minute per run, starting at 200 seconds
...
# = (900-300-100)/50 = 10 epochs per run. Here we have 3*2=6 runs. And indeed, we obtain
...
# every 50 seconds between 300 and 900 seconds, every , and ignoring
