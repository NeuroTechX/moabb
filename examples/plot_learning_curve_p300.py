...
jumping_mean_ivals = [[0.10, 0.139], [0.14, 0.169], [0.17, 0.199],
                      [0.20, 0.229], [0.23, 0.269], [0.27, 0.299],
                      [0.30, 0.349], [0.35, 0.409], [0.41, 0.449],
                      [0.45, 0.499]]
jmv = JumpingMeansVectorizer(fs=processing_sampling_rate,
                             jumping_mean_ivals=jumping_mean_ivals)

pipelines['JM + LDA'] = make_pipeline(jmv,
                                      LDA(solver='lsqr', shrinkage='auto'))
...
evaluation = WithinSessionEvaluationIncreasingData(
    paradigm=paradigm, datasets=datasets, data_size=data_size, n_perms=n_perms,
...
if n_subs > 1:
    r = results.groupby(['pipeline', 'subject', 'data_size'])\
        .mean().reset_index()
else:
    r = results

sns.pointplot(
    data=r, x='data_size', y='score', hue='pipeline', ax=ax, palette="Set1")
